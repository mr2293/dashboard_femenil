...
# Helper: convierte la columna de dolor (texto antiguo O numérico nuevo) a numérico 1–5
parse_dolor_raw <- function(x) {
  # Si ya es numérico (formato nuevo), devolverlo tal cual
  num <- suppressWarnings(as.numeric(x))
  # Si es texto del formato antiguo, convertirlo al equivalente 1–5
  text_map <- case_when(
    x == "No me duele nada"        ~ 1,
    x == "Normal"                  ~ 2,
    x == "Adolorido de una zona"   ~ 3,
    x == "Muy adolorido en general"~ 5,
    TRUE                           ~ NA_real_
  )
  coalesce(num, text_map)
}

# Darle Puntuación a Recuperación de Jugadoras -------------

compute_recovery_score <- function(fatigue, sleep_quality, sleep_hours, soreness) {
  score <- 0
  ...
  # Pain — handles both old text format and new numeric format (1–5)
  dolor_num <- parse_dolor_raw(soreness)
  score <- score + case_when(
    dolor_num == 1 ~ 2.5,   # No me duele nada
    dolor_num == 2 ~ 2.0,   # Normal
    dolor_num == 3 ~ 1.5,   # Algo de dolor
    dolor_num == 4 ~ 1.0,   # Bastante dolor
    dolor_num == 5 ~ 0.5,   # Muy adolorido
    TRUE ~ 0
  )
...
# Pain score: higher number = more pain (for the ACWR x Pain scatter)
# Scale: 1 (no pain) → 4,  5 (max pain) → 8
  ...
  player_long <- player_df |>
    ...
    mutate(
      ...
      respuesta = case_when(
        ...
        score_type == "dolor_score"  ~ as.character(
          `¿Estás adolorida de alguna parte? Indica un valor númerico (1= muy muy poco dolor, 5= muy muy adolorida).`
        )
      ),
      color = case_when(
...
# Example usage:
# plot_player_recuperacion("Geyse Ferreira")
...
plot_individual_ac <- function(player) {
  ...
  ggplot(filtered_data, aes(x = date)) +
    ...
    labs(title = paste("Relación A:C 7:21 –", player), x = NULL,
         y = "Nivel de Carga", color = NULL) +
    geom_text(
      data = tibble(x = max(filtered_data$date) + 0.2,
                    y = ac_breaks * scaling_factor,
                    label = as.character(ac_breaks)),
      aes(x = x, y = y, label = label),
      inherit.aes = FALSE, hjust = 0, size = 5.8, fontface = "bold", color = "black"
    ) +
...
plot_individual_hsr <- function(player) {
  ...
  ggplot(filtered_data, aes(x = date)) +
    ...
    geom_point(aes(y = chronic_HSR, color = "HSR Crónico",
               ...
               size = 3.5) +
    scale_x_date(date_breaks = "1 day", date_labels = "%d-%b") +
    labs(title    = paste("HSR (>21 km/h) Agudo vs. Crónico –", player),
         x        = NULL,
         y        = "HSR en metros",
         color    = NULL,
...
# ACWR x Rest Score & Pain Score Plots -------
